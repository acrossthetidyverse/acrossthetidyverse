...
##Melting Faces with `dplyr` and `purrr`
...
Despite years of sporadic practice, I'm a fairly average musician. I've always been jealous of those ridiculously talented guitar shredders and Jazz freaks who can just root people to the spot, their jaws hanging open, faces exuding pure joy at the brilliance of what they are witnessing.
...
<img src="http://i.giphy.com/MGZP3Kk1GstwI.gif" alt="Math Math Math"></div>

Or perhaps more precisely, this person:

<div style="display: block; margin: auto; width: 50%">
<img src="https://pbs.twimg.com/media/C2GPLjqWQAAswOY.jpg" alt="You're wizard Hadley" style="width:60%;max-height: 60%"></div>
...
`StringAsFactors=FALSE` is defintiely the right way to go, but for modelling you need'll likely need those strings to be factors. Can reverse as variation.
...
##Slick Effect Plot
Parsing linear model effects works great as a vis. A great intro riff for a rant about how rad `broom` is.
```{r, echo=FALSE}
data("ShipAccidents")
...
##Lag It Out
All the lags for all the things!
```{r}
airquality %>% 
  arrange(Month, Day) %>%
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/EEzyrpfrPEI" frameborder="0" allowfullscreen style="margin: auto"></iframe>
